/**
 * example DB lifecycle adapter — gives this app the lifecycle Mongo apps get for free:
 * preflight probe · clean-slate wipe · failure-evidence snapshot. Scaffolded by `touchstone db-adapter example`.
 *
...
const adapter = {
  ...
  validateSafety(env = process.env) {
    ...
    if (vals.every(([, v]) => !v)) return { ok: false, problems: ['no example env configured — set ' + this.envKeys.join(', ') + ' in .env.test'] };
  ...
  async probe() {
    ...
    throw new Error('TODO(db-adapter): implement probe() for example');
  },
  ...
  async wipe() {
    ...
    throw new Error('TODO(db-adapter): implement wipe() for example');
  },

  /** OPTIONAL failure evidence: write relevant state as files into `dir` (cap sizes — evidence, not a backup). */
  async snapshot(dir) {
    // TODO(db-adapter): e.g. writeFileSync(`${dir}/example-docs.json`, JSON.stringify(await dumpSample(), null, 2))
    //                   …or DELETE this method if snapshots don't make sense for example.
    throw new Error('TODO(db-adapter): implement snapshot() for example (or delete this method)');
  },
