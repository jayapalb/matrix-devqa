// Contract self-test for the mqtt DB adapter. KEEP THIS GREEN — Touchstone refuses a non-conformant
...
test('LIVE: probe → wipe → probe round-trip against .env.test', { skip: !configured && 'no mqtt test env configured (.env.test)' }, async () => {
...
test('LIVE: snapshot writes at least one evidence file', { skip: (!configured || typeof adapter.snapshot !== 'function') && 'no test env / no snapshot()' }, async () => {
  const dir = mkdtempSync(join(tmpdir(), 'mqtt-snap-'));
