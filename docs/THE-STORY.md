...
| Every room app names its REPORTING HOST — the thing that can actually run it: a registry **device** (fleet-discovered, bound to its own displays) or the **shell** (micro-apps — reaching barco displays only as a source COMPOSITED by the composer, or shown on the room display). An enabled app with no host is flagged un-runnable (VAL-APP-HOST); a barco decoder is never a host. **The shell now REGISTERS its micro-app list with the registry exactly like an agent** — kind `shell`, same trust path (Ed25519 enroll → identity-signed registration → lease renewal), with two hard guardrails: NO command endpoint (the shell commands, is never commanded), and any display/source claims are stripped at the registry door. Its apps derive with EMPTY displayIds (composited — not window-placeable); on import, an authored `shell` host sentinel upgrades to the discovered deviceId (never the reverse; a device-reported host is never overwritten). Headless API devices (the streamer) surface as plannable CAPABILITIES the planner authors against — never live-commanded by the planner. **[FUTURE]**: main.js wiring (module ships tested + documented; hookup lands with the in-flight shell tree); planner-authored shell app ids (checklist/pacs/endo…) unify with the shell's real manifest appIds | matrix-shell `electron/registry-app-host.js` + registry `shell` kind (normalize strips displays/sources; derive → apps only) + planner `upgradeAppHosts` + MATERIALIZED_KINDS excludes `shell` from API rows + fleet panel | shell module tests 5/5 (byte-compat Ed25519 verify, 403→re-enroll, apps-changed re-register); registry tests 17/17 (hijack-strip, coexist-with-display-agent); api hosting tests 5/5 (sentinel-only upgrade); e2e `make register-shell-apps` drives the REAL shell module against the live stack — 11/11 `SHELL APP REGISTRATION: TRUTH HOLDS` |
