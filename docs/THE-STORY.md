...
left column is what the registry *discovers* right now (read-only facts — id,
kind, host, format — one click to adopt what's new) and beneath it what this
room actually *uses in the plan*. Presence, availability, and hosting are
