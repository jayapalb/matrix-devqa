...
router — the page's layout now says so). Each row opens read-only facts (id,
kind, provider, format), one click adopts what's new, and beneath it sits what
this room has **adopted for planning** — and adoption is not usage: a row the
plan actually references says so right on the row ("in 2 plans", "feeds 1
source"), so "can I delete this?" is answered at a glance, and removal warns
exactly what unbinds only when something truly depends on it. Presence, availability, and hosting are
**live-reconciled, never the stored value**: the old PACS encoder biomed
decommissioned reads `not in registry`, a banner splits the fallout —
*"remove N with no plan impact"* is one click, while anything a plan still
routes is named with a per-row warning of exactly what removal unbinds. The
same drift lands as **warn items in Readiness**, so nobody publishes a plan
that routes dead glass or launches an app no host reports anymore. The
planner only ever *reads* the fleet — retiring hardware happens in the
**Device Registry console**, linked from the page — and agentless clinical
assets (the C-arm, the scope tower) are marked `no agent`: real equipment
that never registers, never counted as drift.
...
| Card ⊕ procedure ⊕ tuning, non-destructive; INTENT FALLBACK IS VISIBLE: a step whose intent no room layout carries still resolves (card's preferred profile, else first layout) but readiness WARNs per missing intent, naming the substitute layout and the affected steps — hand-pinned step layouts stay silent (deliberate choice) | `resolve.ts` + `readinessReport` layout items | contract resolve tests + validate intent-fallback tests (warn names substitute / match silent / pinned silent) |
| Every room app names its REPORTING HOST — the thing that can actually run it: a registry **device** (fleet-discovered, bound to its own displays) or the **shell** (micro-apps — reaching barco displays only as a source COMPOSITED by the composer, or shown on the room display). An enabled app with no host is flagged un-runnable (VAL-APP-HOST); a barco decoder is never a host. **The shell now REGISTERS its micro-app list with the registry exactly like an agent** — kind `shell`, same trust path (Ed25519 enroll → identity-signed registration → lease renewal), with two hard guardrails: NO command endpoint (the shell commands, is never commanded), and any display/source claims are stripped at the registry door. Its apps derive with EMPTY displayIds (composited — not window-placeable); on import, an authored `shell` host sentinel upgrades to the discovered deviceId (never the reverse; a device-reported host is never overwritten). Headless API devices (the streamer) surface as plannable CAPABILITIES the planner authors against — never live-commanded by the planner. **App-id unification DELIVERED**: the shell genuinely hosts the plan vocabulary — checklist / telestration / pacs / endo / conferencing are real matrix-apps manifests (the WHO checklist is a real micro-app, not a placeholder id), so discovery upgrades every authored sentinel and the room's whole app list is discovery-backed (9× shell.or-03 + 2× agent.windows-or03, zero sentinels). **main.js wiring LANDED** (shell 43991bc): the REAL Electron shell self-registers 4s after boot — shell.or-03 online via=lease, no command endpoint, reporting its TRUE 13-app inventory (all five plan-vocabulary apps included) with no harness involved; lease expiry takes it back out of the fleet when the console dies. NOTHING on this seam remains future | matrix-shell `electron/registry-app-host.js` + registry `shell` kind (normalize strips displays/sources; derive → apps only) + planner `upgradeAppHosts` + MATERIALIZED_KINDS excludes `shell` from API rows + fleet panel; the Apps tab's editor collapsed to ROOM APP POLICY (on/off toggles over the discovered list — no hand-authoring path remains); overlay surfaces moved to the LAYOUTS tab (plan vocabulary lives with placement, the Apps tab is pure inventory+policy) with the app→surface cascade now server-owned (cascadeAppRemoval); CAPABILITIES went discovered too (routing provider = whoever REGISTERS a routing capability — the Barco adaptor; the roomDisplay knob is RETIRED for an unconditional console check in readiness live: shell.or-03 present+online from registry truth, asserted by register-shell-apps); the SOURCES tab collapsed the same way (fleet panel + curation-only editing — name/color/device-link/guardrails; identity read-only; import-only, include=sources scope) and system-display windows offer ONLY the owning agent's apps (WindowInspector strict displayIds, VAL-APP-DISP made preventive). Residual: source-id unification (seed alias ids vs canonical src.or3.* + audio channels — 7 'new' in the fleet panel until then) | shell module tests 5/5 (byte-compat Ed25519 verify, 403→re-enroll, apps-changed re-register); registry tests 17/17 (hijack-strip, coexist-with-display-agent); api hosting tests 5/5 (sentinel-only upgrade); e2e `make register-shell-apps` drives the REAL shell module against the live stack — 11 apps discovered from real manifests; with `--import`: added 4 · upgraded 5 (pacs/endo/telestration/checklist/conferencing → shell.or-03; or-vitals-status stays on the PC), 15/15 `TRUTH HOLDS` |
| TWO DISPLAY KINDS are first-class: **barco-decoder** sinks whose slots take routed **sources** (slot capacity modeled — more windows than a decoder's `maxSlots` is a publish **FAIL**, VAL-SLOTS; the Layouts editor speaks each display's language — "+ Slot" with a live n/maxSlots capacity badge and a PREVENTIVE gate at capacity on decoders, "+ Window" on system displays) vs **computer-agent system displays** that host **apps** (VAL-APP-DISP); the demo fleet registers both kinds and the phase preview renders EVERY display with its driver semantics ("Barco · 4 slots (sources)" / "System · hosts apps"). **The demo rig IS the spec**: PC adaptor with a 4K + an HD system display; two barco MNA-420 quad decoders; exactly FOUR clinical barco sources (Endoscope 4K, C-arm 4K, Room Camera HD, PACS HD — plan-alias entries follow the canonicals, and the events-bridge reverse map is first-entry-wins so folds always use canonical ids); ONE speaker + TWO audio sources (music, voice alerts); and a **streaming adaptor** (`streaming-agent-mock`, kind `streamer`, busy-while-live) — same ids the planner authors, so import reads "known" | contract `Display.driver`/`maxSlots` + VAL-SLOTS + `DisplaysPreview` + barco env + `windows-agent-mock` + `streaming-agent-mock` | contract validate tests (overflow fails, system displays exempt); barco-agent bridge tests (canonical-id fold regression, 18/18); streamer **L1-conformant 15/15**, online in-room via MQTT LWT; `make surgery` 31/31 + `make chaos` 12/12 on the new rig (LWT bar scoped to COMMANDABLE devices — the shell's lease presence asserted separately: it must never fake lwt) |
...
| Roster pages tell FLEET TRUTH (Devices / Sources / Apps, one IA): "Discovered (N)" = the registry's live registrations, browsable read-only facts, GROUPED BY PROVIDER (the registering agent — endpoints never register themselves; the registry stamps `providedBy` on every derived display/source/speaker so the dependency is data, not inference); PLAN-RELEVANT FACTS RIDE THE REGISTRATION AND ARE **DISCOVERED LIVE FROM THE VENDOR NMS** (barco-agent `discoverNms` → Nexxis 2.0.0 GraphQL `videoSinks`: `videoSignal.currentTiming`/EDID resolution → 4K vs HD, `capabilities.maxAllowedSlots` → `Display.maxSlots` feeding VAL-SLOTS, `displayInfo.model` → the real display model): DISCOVERY OWNS FACTS, COMMISSIONING OWNS MEANING (vendor id → planner displayIds/sourceId, nurse labels, audio roles `music`/`voice-alert`/`mic`); vendor ids never leak up — uncommissioned NMS sinks are reported, never auto-registered; NMS down → last-known facts stand; encoder feeds carry `format`; "Adopted for planning (N)" = the adopted working set (adoption ≠ usage — plan-referenced rows carry an inline usage note from the same warn*Delete analysis), curation-only (identity belongs to the reporting device/host). Presence, availability, and hosting are live-reconciled — the stored value is never rendered; drift (adopted but no longer registered) gets a banner that SPLITS safe-remove: "remove N with no plan impact" bulk vs plan-used rows named with per-row cascade warnings. Device+source+app drift also = readiness WARNs when plan-used (layout windows, role bindings, step routes/overlays, window app regions); `noAgent` assets (C-arm, scope tower — never registered by design) are exempt everywhere incl. publish expected-inventory. The planner READS the fleet only — retire/deregister lives in the registry console, linked from each page | planner `DevicesForm`/`SourcesForm`/`AppsForm` + `warn*Delete` impact analysis + contract `withDeviceDriftReadiness`/`withSourceDriftReadiness`/`withAppDriftReadiness` + readiness route | contract drift tests (plan-used warns / unused silent / noAgent exempt / app window-region + plan-bound overlay refs) + live browser verification on the dev stack |
