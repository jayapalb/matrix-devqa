...
| TWO DISPLAY KINDS are first-class: **barco-decoder** sinks whose slots take routed **sources** (slot capacity modeled — more windows than a decoder's `maxSlots` is a publish **FAIL**, VAL-SLOTS) vs **computer-agent system displays** that host **apps** (VAL-APP-DISP); the demo fleet registers both kinds (2× 4K quad-slot barco sinks + a 4K and an HD composer display — same ids the planner authors, so import reads "known") and the phase preview renders EVERY display with its driver semantics ("Barco · 4 slots (sources)" / "System · hosts apps") | contract `Display.driver`/`maxSlots` + VAL-SLOTS + `DisplaysPreview` + barco env + `windows-agent-mock` | contract validate tests (overflow fails, system displays exempt); barco-agent bridge tests (canonical-id fold regression, 18/18); `make surgery` 30/30 + `make chaos` 12/12 after the topology change |
